/* PID balance code with ping pong ball and input sensor sharp 2y0a21
...
float setpoint = 2000;           //Should be the input from sensor to the middle of the bar in mm
...
void loop() {
  if (millis() > time)
  {
    ...
    PID_p = kp * error;
    float dist_diference = error - last_error;     
    PID_d = kd*(error - last_error);
      
    if(-2000 < error && error < 2000)
    {
...
//
//  float input_cm = 17569.7 * pow(adc, -1.2062);
//  //float input_cm = 13*pow(volts, -1); 
//  return(input_cm);
//}
