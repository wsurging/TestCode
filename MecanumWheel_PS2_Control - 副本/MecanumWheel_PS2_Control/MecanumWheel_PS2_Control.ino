...
void loop() {
  ...
  */
  ...
  if (ps2x.Button(PSB_L1) || ps2x.Button(PSB_R1)) { //drive motor if either is TRUE
    ...
    Serial.println();
#endif
    moveMotor(speedm_y + speedm_x + speedm_w, (speedm_y + 200) - (speedm_x + 200) - speedm_w, (speedm_y + 100) - (speedm_x + 100) + speedm_w, (speedm_y + 200) + (speedm_x + 200) - speedm_w); // 电机M1,M2,M3,M4 运动
  }
...
void m4_phase_a(void) {
  //  bool nowM4PhaseA = digitalRead(M4PHASEAPIN);
  //  if (nowM4PhaseA)
  //    M4PulseValue--;
  //  else
  M4PulseValue++;
