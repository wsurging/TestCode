...
int SERVOMIDIA = 150;       //舵机初始开机复位角度
int SERVOUPIA = 90;         //舵机初始开机复位角度
int SERVOPAWIA = 90;        //手爪初始角度
int SERVOPAWTA1 = 140;      //手爪闭合角度
...
void loop() {
  ...
  ***********************************************************************/
  ...
  } else if (0 <= ps2x.Analog(PSS_LY) && ps2x.Analog(PSS_LY) <= 120 && 0 <= ps2x.Analog(PSS_RX) && ps2x.Analog(PSS_RX) <= 120) {
    EnSLP();
    digitalWrite(DIRA, LOW);
    digitalWrite(DIRB, HIGH);
    analogWrite(PWMA, map(ps2x.Analog(PSS_RX), 0, 120, 0, MAXSPEEDA));
    analogWrite(PWMB, map(ps2x.Analog(PSS_LY), 0, 120, MAXSPEEDB, 0));
  } else if (0 <= ps2x.Analog(PSS_LY) && ps2x.Analog(PSS_LY) <= 120 && 255 >= ps2x.Analog(PSS_RX) && ps2x.Analog(PSS_RX) >= 135) {
    EnSLP();
    digitalWrite(DIRA, LOW);
    digitalWrite(DIRB, HIGH);
    analogWrite(PWMA, map(ps2x.Analog(PSS_LY), 0, 120, MAXSPEEDA, 0));
    analogWrite(PWMB, map(ps2x.Analog(PSS_RX), 135, 255, MAXSPEEDB, 0));
  } else {
    Stop();
  }
